...
function upload($UploadFileField,$TargetDirectory,$typeArray){
	...
	if(!$UploadTmp){
		die ("No File Selected, Please Upload Again <p><a style=\"color:white\" href=\"prof_research_topic_form.php\">Return</a></p>");	
	}else{
...
if(isset($_POST["title"]) && isset($_FILES[$pdf]) && isset($_FILES[$img]) && isset($_POST["summary"]) && $_POST['title'] != "" && $_POST['summary'] != ""){
	...
	if($ret === false) {
		...
		echo "$ret bytes written for $my_file";
	}	
...
<?php 
	...
	echo"
		<form name=\"FileUploadForm\" id=\"FileUploadForm\" enctype=\"multipart/form-data\" action=\"prof_add_research_topic_form.php\" method=\"POST\">
			Research Topic Title: <input type=\"text\" name=\"title\" id=\"title\" /><br/>
			...
			PDF or HTML file: <input type=\"file\" id=\"".$pdf."\" name=\"".$pdf."\"/><br/>
