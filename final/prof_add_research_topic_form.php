<?php
	...
	$banner_writing="<h1>Add a Research Topic</h1>";
...
$returnLink="<p><a style=\"color:white\" 
		href=\"prof_add_research_topic_form.php\">
		Return
		</a></p>";
...
function upload($UploadFileField,$TargetDirectory,$typeArray){
	...
	die ($_FILES[$UploadFileField]['name']." already exists in $targetFile. 
	...
	if(!$goodFileType){
		...
		die($TargetDirectory." must be of the following type(s) (".$msg.")<p><a style=\"color:white\" 
		href=\"prof_add_research_topic_form.php\">
		Return
		</a></p>");	
	}
	...
	if(!$UploadTmp){
		die ("No File Selected, Please Upload Again <p><a style=\"color:white\" href=\"prof_add_research_topic_form.php\">Return</a></p>");	
	}else{
...
if(isset($_POST["title"]) && isset($_FILES[$pdf]) && isset($_FILES[$img]) && isset($_POST["summary"]) && $_POST['title'] != "" && $_POST['summary'] != ""){
	...
	//upload pdf
	if(isset($_FILES[$pdf])){
		upload($pdf,"PDFs/",array("pdf","html"));
	}
...
<?php 
	...
	echo"
		<form name=\"FileUploadForm\" id=\"FileUploadForm\" enctype=\"multipart/form-data\" action=\"prof_add_research_topic_form.php\" method=\"POST\">
			Research Topic Title: <input type=\"text\" name=\"title\" id=\"title\" placeholder=\"Research Topic Title\"/><br/>
			Tag Image: <input type=\"file\" id=\"".$img."\" name=\"".$img."\"/><br/>
			Research Topic Summary: <textarea name=\"summary\" placeholder=\"Please summarize Research Topic here.\"></textarea></br>
			PDF file: <input type=\"file\" id=\"".$pdf."\" name=\"".$pdf."\"/><br/>
			<input type=\"submit\" value=\"Upload New Topic\"/>
		</form>";
