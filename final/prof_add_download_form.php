<?php
	$page_title="AddDownload";
	$banner_id="banner_none";
	$banner_writing="<h1>Add a Download</h1>";
...
$returnLink='<p><a style=\"color:white\" 
		href=\"prof_add_download_form.php\">
		Return
		</a></p>';
...
function upload($UploadFileField,$TargetDirectory,$typeArray){
	...
	if(!$UploadTmp){
		die ("No File Selected, Please Upload Again <p><a style=\"color:white\" href=\"prof_add_dowload_form.php\">Return</a></p>");	
	}else{
...
if(isset($_POST["title"]) && isset($_FILES[$pdf]) && isset($_FILES[$img]) && isset($_POST["summary"]) && $_POST['title'] != "" && $_POST['summary'] != ""){
	...
	"D",///////////////////////////////////////////////////THIS IS THE ONLY 'important' CHANGE BETWEEN THESE ADD FILES
...
?>
<main>
	<h2>Add Download Information</h2>
<?php 

	echo"
		<form name=\"FileUploadForm\" id=\"FileUploadForm\" enctype=\"multipart/form-data\" action=\"prof_add_download_form.php\" method=\"POST\">
			Download Title: <input type=\"text\" name=\"title\" id=\"title\" placeholder=\"Download Title\"/><br/>
			Tag Image: <input type=\"file\" id=\"".$img."\" name=\"".$img."\"/><br/>
			Download Summary: <textarea name=\"summary\" placeholder=\"Please give a description of the downloadable content here. URLs can be placed here as well (example: http://www.jalview.org/Download). \"></textarea></br>
			Select a PDF/Excel/PowerPoint/Word/.txt file: <input type=\"file\" id=\"".$pdf."\" name=\"".$pdf."\"/><br/>
			<input type=\"submit\" value=\"Upload New Download\"/>
		</form>";
