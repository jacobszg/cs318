<?php
	$page_title="AddEquipment";
	$banner_id="banner_none";
	$banner_writing="<h1>Add Equipment</h1>";
...
$returnLink='<p><a style=\"color:white\" 
		href=\"prof_add_equipment_form.php\">
		Return
		</a></p>';
...
function upload($UploadFileField,$TargetDirectory,$typeArray){
	...
	if(!$UploadTmp){
		die ("No File Selected, Please Upload Again <p><a style=\"color:white\" href=\"prof_add_equipment_form.php\">Return</a></p>");	
	}else{
...
if (isset($_POST['title']) && !($_POST['title'] == "") && !(isset($_FILES[$pdf]))){
	die ("The PDF file is to big.
...
if(isset($_POST["title"]) && isset($_FILES[$pdf]) && isset($_FILES[$img]) && isset($_POST["summary"]) && $_POST['title'] != "" && $_POST['summary'] != ""){
	...
	"Q",///////////////////////////////////////////////////THIS IS THE ONLY 'important' CHANGE BETWEEN THESE ADD FILES
...
<?php 
	...
	echo"
		<form name=\"FileUploadForm\" id=\"FileUploadForm\" enctype=\"multipart/form-data\" action=\"prof_add_equipment_form.php\" method=\"POST\">
			Equipment Title: <input type=\"text\" name=\"title\" id=\"title\" placeholder=\"Equipment Title\"/><br/>
			Tag Image: <input type=\"file\" id=\"".$img."\" name=\"".$img."\"/><br/>
			Equipment Summary: <textarea name=\"summary\" placeholder=\"Summarize equipment use and expectations here.\"></textarea></br>
			Select a file: <input type=\"file\" id=\"".$pdf."\" name=\"".$pdf."\"/><br/>
			<input type=\"submit\" value=\"Upload New Equipment\"/>
		</form>";
