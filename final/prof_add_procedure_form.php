<?php
	$page_title="AddProcedure";
	$banner_id="banner_none";
	$banner_writing="<h1>Add a Procedure</h1>";
...
$returnLink='<p><a style=\"color:white\" 
		href=\"prof_add_procedure_form.php\">
		Return
		</a></p>';
...
function upload($UploadFileField,$TargetDirectory,$typeArray){
	...
	if(!$UploadTmp){
		die ("No File Selected, Please Upload Again <p><a style=\"color:white\" href=\"prof_add_procedure_form.php\">Return</a></p>");	
	}else{
...
if(isset($_POST["title"]) && isset($_FILES[$pdf]) && isset($_FILES[$img]) && isset($_POST["summary"]) && $_POST['title'] != "" && $_POST['summary'] != ""){
	...
	"P",///////////////////////////////////////////////////THIS IS THE ONLY 'important' CHANGE BETWEEN THESE ADD FILES
...
<?php 
	...
	echo"
		<form name=\"FileUploadForm\" id=\"FileUploadForm\" enctype=\"multipart/form-data\" action=\"prof_add_procedure_form.php\" method=\"POST\">
			Procedure Title: <input type=\"text\" name=\"title\" id=\"title\" placeholder=\"Procedure Title\"/><br/>
			Tag Image: <input type=\"file\" id=\"".$img."\" name=\"".$img."\"/><br/>
			Procedure Summary: <textarea name=\"summary\" placeholder=\"Summarize the procedure here.\"></textarea></br>
			Select a PDF file: <input type=\"file\" id=\"".$pdf."\" name=\"".$pdf."\"/><br/>
			<input type=\"submit\" value=\"Upload New Procedure\"/>
		</form>";
